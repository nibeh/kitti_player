            ...
            (1.0 / 2.0 +
             A * A *
                 ((5.0 - T + 9.0 * C + 4.0 * C * C) / 24.0 +
                  A * A * (61.0 - 58.0 * T + T * T + 600.0 * C - 330.0 * e0sq) /
                      720.0))));
  if (y < 0) {
 ...
 * (=0) view loaded disparity images -F [ --frame      ] [=arg(=0)] (=0) start
 * playing at frame ...
 *
  ...
  if (options.viewer) {
    ...
    if (options.color || options.all_data) {
      ROS_DEBUG_STREAM("color||all " << options.color << " "
                                     << options.all_data);
      cv::namedWindow("CameraSimulator Color Viewer", cv::WINDOW_AUTOSIZE);
    ...
    if (options.grayscale || options.all_data) {
      ROS_DEBUG_STREAM("grayscale||all " << options.grayscale << " "
                                         << options.all_data);
      cv::namedWindow("CameraSimulator Grayscale Viewer", cv::WINDOW_AUTOSIZE);
    ...
    if (options.viewDisparities || options.all_data) {
      ROS_DEBUG_STREAM("viewDisparities||all " << options.grayscale << " "
                                               << options.all_data);
      cv::namedWindow("Precomputed Disparities", cv::WINDOW_AUTOSIZE);
      full_filename_Disparities =
          dir_Disparities + boost::str(boost::format("%010d") % 0) + ".png";
      cv_disparities =
          cv::imread(full_filename_Disparities, cv::IMREAD_UNCHANGED);
      cv::waitKey(5);
    }
  ...
  do {
    ...
    if (options.viewDisparities) {
      ...
      cv::putText(cv_disparities, "KittiPlayer", cvPoint(20, 15),
                  cv::FONT_HERSHEY_SIMPLEX, 0.4, cvScalar(0, 255, 0), 1,
                  cv::LINE_AA);
      cv::putText(
          cv_disparities, boost::str(boost::format("%5d") % entries_played),
          cvPoint(cv_disparities.size().width - 100, 30),
          cv::FONT_HERSHEY_DUPLEX, 1.0, cvScalar(0, 0, 255), 1, cv::LINE_AA);
      cv::waitKey(5);
    }
